...
import { formatLWPDate, logError } from "./utils.js";
...
export class CookieExporter {
  ...
  private static getAtCoderCliSessionPath(): string {
    try {
      ...
      throw new Error(
        `Failed to get atcoder-cli config directory: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async getAtCoderCookies(): Promise<PlaywrightCookie[]> {
    ...
    const atcoderCookies = sessionData.cookies.filter(
      (cookie) =>
        cookie.domain.includes("atcoder.jp") && (cookie.name === "REVEL_FLASH" || cookie.name === "REVEL_SESSION"),
    );

    if (atcoderCookies.length === 0) {
      throw new Error(
        "No AtCoder cookies (REVEL_FLASH or REVEL_SESSION) found. Make sure you are logged in to AtCoder in the browser",
      );
    }
  ...
  async exportCookiesForAtCoderCli(): Promise<void> {
    try {
      ...
      logError("atcoder-cli cookie export", error);
    }
  }
  ...
  async exportCookies(cookiePath: string): Promise<void> {
    try {
      ...
      logError("oj cookie export", error);
    }
  }
