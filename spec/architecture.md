...
- **Universal Generator**: 設定ファイル (`.json5`) に基づき、言語に依存しない共通の生成ロジック（入力処理の構築、引数リスト作成等）を実行。
- **Language Generators**: `CPlusPlusGenerator` 等の言語別のクラスが、`UniversalGenerator` と Nunjucks テンプレートを組み合わせて最終的なコードを出力。
